...
- **goal-6 — MVP layout: pinned lists + cross-list drag** *(next; brief `goal-6.md`)*: full-width top row `My Tasks | Follow-up | Scratchpad` (scratchpad rightmost, enlarged); pinned lists static-in-code by title; drag tasks **between** the pinned lists (reuses the g4 `move` write layer — proposed: `move` gains optional `due_date` so a cross-list + cross-bucket drop is one orchestrated write); shared `DndContext` across the pinned pair; all other lists in a collapsed **Other tasks** section; calendar below the fold. No `ui_prefs` yet.
